...
clear all;
close all;
addpath('/media/saumya/Data/Study/CMU/Robotics/PlanningWithContact/Codes/Planning_with_contact_to_reduce_uncertainty/BSP_iterative_local_optimization/WithObstacles_measurementAroundObstacles/utils');
...
Qt = 1000*eye(nState);  % sigma*Qt*sigma
Ct = 700; % penalty for obstacle avoidance
Qf = 1000*N*eye(nState); %x'*Qf*x + sigma*Qf*sigma
...
for i = 1:length(mapx)
    for j = 1 : length(mapy)
        contZvar(i,j) = measurementVarianceMap(mapx(i),mapy(j));
    end
end
