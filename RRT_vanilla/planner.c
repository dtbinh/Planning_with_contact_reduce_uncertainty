...
void BuildRRT(double* armstart_anglesV_rad, double* armgoal_anglesV_rad, int maxconfigs, float step_size, double*  map, int numofDOFs, double*** plan, int x_size, int y_size, int** parent){
  state *qnear;
  qnear = new state;
  state *qnew;
  qnew = new state;
  ...
  for (int i = 1; i <=maxconfigs; i++){
        ...
        qrand = RandRRT(armgoal_anglesV_rad, goalProbability);
        qnear = nearestNeighborRRT(armstart_anglesV_rad, qrand, qnear, nearDist);
        qnew = extendRRT(qnear, qrand, step_size);
        if(!IsValidArmConfiguration(qnew, numofDOFs, map, x_size, y_size) && distNorm(qnew, armgoal_anglesV_rad) < step_size && !IsValidArmConfiguration(armgoal_anglesV_rad, numofDOFs, map, x_size, y_size))
        {
            //cout<<"Goal reached\n";
            printf("ERROR: Invalid arm configuration!!!\n");
            (*parent)[i] = 
        }
        if (collisionFree(qnew->q) == 1 && distNorm(qnew->q, qgoal->q) < step_size && collisionFree(qgoal->q) == 1)
        {
          //cout<<"Goal reached\n";
          qgoal->PREV = qnew;
          qnew->NEXT.push_back(qgoal);
          return;
        }
	   ...
	   int* planlength)
{
  ...
  int maxconfigs = 1000;
  BuildRRT(armstart_anglesV_rad, armgoal_anglesV_rad, maxconfigs, step_size, map, numofDOFs, plan, x_size, y_size, parent);
/*    double distance = 0;
    int i,j;
